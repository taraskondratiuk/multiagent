...
public class CitizenAgent extends Agent {
    ...
    protected void setup() {
        ...
        this.priceForMaxTrip = (int) Location.distanceBetweenLocations(new Location(0, 0), new Location(xBound, yBound)) * this.pricePerKm;
        ...
        addBehaviour(new TickerBehaviour(this, 60 * 12) { //switching state each 12 hours
            ...
            protected void onTick() {
                if (stateSwitches < Constants.PERIOD_DAYS * 2) {
                    switchState();
...
//                    System.out.println("citizen " + id + " time waiting " + timeWaitingDriver);
    ...
    private class CitizenGuestBehaviour extends CyclicBehaviour {
        ...
        private int thisRideTimeWaiting = 0;
        private int stage = 0;
        private int repliesCount = 0;
        private long startWaiting;
        @Override
        public void action() {
//            System.out.println("money: " + money + " needed " + (priceForMaxTrip * 2));
            if (!isBusy(System.currentTimeMillis()) && (stage > 0 || state.equals("guesting")) && (money > priceForMaxTrip * 2)) {
                switch (stage) {
                    case 0:
                        startWaiting = 0;
                    ...
                    case 4:
                        ...
                        if (reply != null && reply.getContent() != null) {
                            ...
                            if (repliesCount >= citizens.length) {
                                ...
                                thisRideTimeWaiting += Location.minutesBetweenLocations(nearestLoc, home);
                                if (startWaiting > 0) {
                                    timeWaitingDriver += (System.currentTimeMillis() - startWaiting);
                                    thisRideTimeWaiting += (System.currentTimeMillis() - startWaiting);
                                    startWaiting = 0;
                                }
                                // count hate level
                                if (thisRideTimeWaiting < 60) {
                                    ACLMessage confirm = new ACLMessage(ACLMessage.CFP);
                                    confirm.addReceiver(nearestDriver);
                                    confirm.setContent(Constants.DRIVER_CONFIRM + " " + home + " " + l);
                                    myAgent.send(confirm);

                                    location = l;
                                    money -= (int) Location.distanceBetweenLocations(home, l) * pricePerKm;
                                    repliesCount = 0;
                                    stage = 5;
                                    thisRideTimeWaiting = 0;
                                } else {
                                    thisRideTimeWaiting = 0;
                                    location = home;
                                    repliesCount = 0;
                    ...
                    case 7:
                        ...
                        confirm.addReceiver(nearestDriver);
                        confirm.setContent(Constants.DRIVER_CONFIRM + " " + location + " " + home);
                        myAgent.send(confirm);
                        ...
                        location = home;
                        money -= Location.distanceBetweenLocations(location, home) * pricePerKm;
                        repliesCount = 0;
                        stage = 0;
                        break;
                }
